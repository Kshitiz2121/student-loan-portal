#!/usr/bin/env python3
"""
Simple test setup for Student Loan Portal
"""
...
def create_test_users():
    ...
    try:
        financier, created = FinancierUser.objects.get_or_create(
        ...
        if created:
            financier.set_password('testpass123')
            financier.save()
            print(f"Created financier: {financier.get_full_name()}")
        else:
            print(f"Financier exists: {financier.get_full_name()}")
    except Exception as e:
        print(f"Financier error: {e}")
